...
var sumBelow = function(n) {
	...
	else if (n > 1) return sumBelow(n-1) + n-1;
	else return sumBelow(n+1) + n+1;
};
...
var exponent = function(base, exp) {
	...
	else if (exp > 1) return base*exponent(base,exp-1);
...
var palindrome = function(string) {
	...
	var rec = reverse(str.slice(1,str.length-1));
	var first = str.charAt(0);
	var last = str.charAt(str.length-1);
	return (palindrome(rec) && first === last);
};
...
var modulo = function(x, y) {
  let ptvY = y < 0 ? -y : y;
  let ptvX = x < 0 ? -x : x;
  if (ptvX < ptvY) return x;
  else if (x > 0) return modulo(x - y, y);
  else if (x < 0) return modulo(x + ptvY, y);
